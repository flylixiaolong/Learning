	...
	if(inode)
	{
		...
		switch(mode & S_IFMT)
		{
			...
			case S_IFREG:
				...
				break;
			case S_IFDIR:
				inode->iop = &simple_dir_inode_operations;
...
{
	in tres;
	res = sufs_mknod(dir, dentry, mode|S_IFDIR, 0);
	if(!res)
		dir->i_nlink++;
	return res;
}

static int sufs_create(struct inode *dir, struct dentry *dentry, int mode)
{
...
static int aufs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	return simple_fill_supper(sb, AUFS_MAGIC, debug_files);
}
...
struct dentry *aufs_create_dir(const char *name, struct dentrty *parent)
{
	return aufs_create_file(name,S_IFDIR|S_IRWXU|S_IRUGO|S_IXUGO,
			parent, NULL, NULL);
}

static int __init sufs_init(void)a
{
	int retval;
	struct dentry *pslot;
	...
	if(!retval)
	{
		...
		if(IS_ERR(aufs_mount))
		{
			...
			unregister_filesysstem(&au_fs_type);
			return retval;
		}
	}
	...
	aufs_create_file("lhd", S_IFREG|S_IRUGO, pslot, NULL, NULL);
...
MODULE_LISENSE("GPL");
MODULE_DESCRIPTION("This is a simple moudle.");
MOUDLE_VERSION("v0.1");
